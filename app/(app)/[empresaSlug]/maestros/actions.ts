...
export async function guardarCategoria(formData: FormData): Promise<void> {
  ...
  try {
    ...
    const esImpuestoIndirecto = formData.get('esImpuestoIndirecto') === '1';
    ...
    if (id) {
      ...
      await ctx.db.categoria.update({ where: { id }, data: { nombre, tipo, padreId, esCostoPersonal, esImpuestoIndirecto } });
      await writeAudit(ctx.db, { usuarioId: ctx.usuario.id, entidad: 'Categoria', entidadId: id, accion: 'EDITAR', antes, despues: { nombre, tipo, padreId, esCostoPersonal, esImpuestoIndirecto } });
    } else {
      const nueva = await ctx.db.categoria.create({ data: { nombre, tipo, padreId, esCostoPersonal, esImpuestoIndirecto } as never });
      await writeAudit(ctx.db, { usuarioId: ctx.usuario.id, entidad: 'Categoria', entidadId: nueva.id, accion: 'CREAR', despues: { nombre, tipo, padreId, esCostoPersonal, esImpuestoIndirecto } });
    }
  } catch (err) {
